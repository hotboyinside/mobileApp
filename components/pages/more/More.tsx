...
import { useModal } from '@/components/appProvider/modal/ModalProvider';
...
import PurchaseSuccessBottomSheet from '../premiumModal/PurchaseSuccessBottomSheet';
...
export default function More() {
	...
	const { connected, availablePurchases, restorePurchases, finishTransaction } =
		useIAP();
	const { openSheetModal, closeSheetModal } = useGlobalSheet();
	const restoreAttemptCompleted = useRef(false);
	...
	const expirationDate =
		session?.currentSubscription.mobileSubscription?.expirationDate ||
		session?.currentSubscription.nextBillingDate;
	...
	const restore = async () => {
		...
		restoreAttemptCompleted.current = false;
		await restorePurchases();
		restoreAttemptCompleted.current = true;
	};
	...
	useEffect(() => {
		if (!availablePurchases || availablePurchases.length === 0) {
			if (restoreAttemptCompleted.current) {
				Alert.alert('No purchases to restore');
			}
			return;
		}

		availablePurchases.forEach(async p => {
			try {
				const result = await postInAppPurchasesRequest(p);
